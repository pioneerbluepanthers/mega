...
from fairseq.models.ecg.mega_ecgraw_encoder import MegaECGRawEncoder
...
@register_model('ecg_raw')
class ECGRawModel(FairseqEncoderModel):
    """
...
class ECGRawEncoder(FairseqEncoder):
    ...
    def __init__(self, args, task):
        ...
        if args.layer_type == 'mega':
            self.encoder = MegaECGRawEncoder(
...
@register_model_architecture('ecg_raw', 'mega_ecg_raw')
def mega_lra_ecg(args):
    base_architecture(args)


@register_model_architecture('ecg_raw', 'mega_ecg_raw_base')
def mega_lra_ecg_base(args):
    mega_lra_ecg(args)


@register_model_architecture('ecg_raw', 'mega_ecg_raw_big')
def mega_lra_ecg_big(args):
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
